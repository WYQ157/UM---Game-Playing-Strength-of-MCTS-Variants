...
//特征工程、模型训练和推理都在这个类中编写。
//1.根据数据介绍页面上的描述，构建了agent1和agent2的特征。
//代理字符串描述 所有训练和测试数据中的代理字符串描述均采用以下格式：MCTS----，其中：是以下之一：UCB1、UCB1GRAVE、ProgressiveHistory、UCB1Tuned。这些是在MCTS算法的选择阶段中可能使用的不同策略。
//EXPLORATION_CONST是以下之一：0.1、0.6、1.41421356237。这是我们为“探索常数”测试的三个不同值（一个在所有测试的选择策略中共享的数值超参数）。
//PLAYOUT是以下之一：Random200、MAST、NST。这些是在MCTS算法的播放阶段中可能使用的不同策略。
//SCORE_BOUNDS是以下之一：真或假，指示是否存在“MCTS的分数限制”版本（一个可以证明其搜索树中的某些节点在完美游戏下是胜利/失败/平局的算法版本，并据此调整搜索）。
//例如，使用UCB1GRAVE选择策略、0.1的探索常数、NST播放策略和分数限制的MCTS代理，将被描述为MCTS-UCB1GRAVE-0.1-NST-true。
//2.onehot和tfidf很简单。为了防止数据泄漏，在交叉验证中对每个训练和验证集进行了单独的tfidf处理。
//3.从手动选择和模型选择中移除了一些无用的特征。
...
//groupkfold：本次比赛的最终目标是预测代理对未知游戏的影响，因此我们在交叉验证时需要根据游戏类型进行划分。因此，我们在这里选择了groupkfold。
//后处理说明：
//1. 最小化 𝑅𝑀𝑆𝐸(𝑊∗𝑜𝑜𝑓+𝑏,𝑡𝑎𝑟𝑔𝑒𝑡)：一般来说，每个人默认w=1和b=0，但实际上，很多时候我们可以找到更好的w和b，因此我们需要进行后处理。
//2. clip(pred,-0.985,0.985)：例如，如果我们的模型将数据[-1,1]都预测为1，假设模型将这两个数据预测为相同的值，那么显然当预测为0时，RMSE是最小的。当预测结果从1移动到0时，第二个数据的RMSE在增加，但增加的速度不如第一个数据下降的速度快，最终导致它们的整体RMSE下降。虽然clip操作会增加真正为-1或1的数据的RMSE，但对于真正为其他值但预测为1的RMSE则会减少。
import os
